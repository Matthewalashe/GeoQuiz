/**
 * Generates p3_seed.sql from the JS data files.
 * Run: node supabase/generate_seed_sql.mjs
 */
...
const lines = []
lines.push('-- ============================================================')
lines.push('-- P3 Seed Data — Auto-generated by generate_seed_sql.mjs')
lines.push('-- Paste this into Supabase → SQL Editor and run')
lines.push('-- ============================================================')
lines.push('')

// ═══════════════════════════════════════
// 1. TRIVIA PACKS (inline from seed.js)
// ═══════════════════════════════════════
const triviaRaw = [
  // Lagos pack
  ...[ 
...
lines.push('-- 1. TRIVIA PACKS')
lines.push('INSERT INTO cms_trivia_packs (pack_id, label, description, color, question, options, answer_index, fact, sort_order) VALUES')
const triviaValues = triviaRaw.map(t =>
  `(${esc(t.pack_id)}, ${esc(t.label)}, ${esc(t.description)}, ${esc(t.color)}, ${esc(t.question)}, ${jsonb(t.options)}, ${t.answer_index}, ${esc(t.fact)}, ${t.sort_order})`
)
lines.push(triviaValues.join(',\n') + ';')
lines.push('')
...
lines.push('-- 2. CROSSWORDS')
lines.push('INSERT INTO cms_crosswords (puzzle_id, label, description, color, grid_size, across, down, sort_order) VALUES')
const cwValues = crosswords.map(c =>
  `(${esc(c.puzzle_id)}, ${esc(c.label)}, ${esc(c.description)}, ${esc(c.color)}, ${c.grid_size}, ${jsonb(c.across)}, ${jsonb(c.down)}, ${c.sort_order})`
)
lines.push(cwValues.join(',\n') + ';')
lines.push('')
...
lines.push('-- 3. PUZZLE IMAGES')
lines.push('INSERT INTO cms_puzzles (puzzle_id, label, image, sort_order) VALUES')
const pzValues = puzzles.map(p =>
  `(${esc(p.puzzle_id)}, ${esc(p.label)}, ${esc(p.image)}, ${p.sort_order})`
)
lines.push(pzValues.join(',\n') + ';')
lines.push('')
...
lines.push('-- 4. CAMPAIGN CHAPTERS')
lines.push('INSERT INTO cms_campaign (chapter_id, week, title, subtitle, icon, color, badge, badge_emoji, intro, category_filter, question_count, stages, facts, sort_order) VALUES')
const campValues = campaign.map(c =>
  `(${esc(c.chapter_id)}, ${c.week}, ${esc(c.title)}, ${esc(c.subtitle)}, ${esc(c.icon)}, ${esc(c.color)}, ${esc(c.badge)}, ${esc(c.badge_emoji)}, ${esc(c.intro)}, ${esc(c.category_filter)}, ${c.question_count}, ${jsonb(c.stages)}, ${jsonb(c.facts)}, ${c.sort_order})`
)
lines.push(campValues.join(',\n') + ';')
lines.push('')
...
lines.push('-- 5. COLORING SCENES')
lines.push('INSERT INTO cms_coloring (scene_id, title, description, color, min_parts, parts, guide, svg_key, sort_order) VALUES')
const colorValues = coloring.map(c =>
  `(${esc(c.scene_id)}, ${esc(c.title)}, ${esc(c.description)}, ${esc(c.color)}, ${c.min_parts}, ${jsonb(c.parts)}, ${jsonb(c.guide)}, ${esc(c.svg_key)}, ${c.sort_order})`
)
lines.push(colorValues.join(',\n') + ';')
lines.push('')

// ═══════════════════════════════════════
// 6. ADVENTURES — import from data file
// ═══════════════════════════════════════
const advModule = await import(pathToFileURL(ROOT + '/src/data/adventures.js').href)
const ADVENTURE_STORIES = advModule.ADVENTURE_STORIES

lines.push('-- 6. ADVENTURE STORIES')
lines.push('INSERT INTO cms_adventures (story_id, title, description, color, start_node, nodes, sort_order) VALUES')
const advEntries = Object.entries(ADVENTURE_STORIES)
const advValues = advEntries.map(([key, s], i) =>
  `(${esc(key)}, ${esc(s.title)}, ${esc(s.desc)}, ${esc(s.color)}, ${esc(s.start)}, ${jsonb(s.nodes)}, ${i})`
)
lines.push(advValues.join(',\n') + ';')
lines.push('')

// ═══════════════════════════════════════
// 7. WORD GAME — import from data file
// ═══════════════════════════════════════
const wordModule = await import(pathToFileURL(ROOT + '/src/data/wordgame.js').href)
const WORD_DATA = wordModule.WORD_DATA

lines.push('-- 7. WORD GAME')
lines.push('INSERT INTO cms_wordgame (word_id, word, clue, category, image, description, history, footnotes, sort_order) VALUES')
const wordValues = WORD_DATA.map((w, i) =>
  `(${esc(w.id)}, ${esc(w.word)}, ${esc(w.clue)}, ${esc(w.category || null)}, ${esc(w.image || null)}, ${esc(w.description || null)}, ${jsonb(w.history || [])}, ${jsonb(w.footnotes || [])}, ${i})`
)
lines.push(wordValues.join(',\n') + ';')
lines.push('')

// ═══════════════════════════════════════
// 8. POSTCARDS — import from data file
// ═══════════════════════════════════════
const postcardModule = await import(pathToFileURL(ROOT + '/src/data/postcards.js').href)
const POSTCARD_QUESTIONS = postcardModule.POSTCARD_QUESTIONS

lines.push('-- 8. POSTCARDS')
lines.push('INSERT INTO cms_postcards (postcard_id, pack, image, question, options, correct_index, category, fact, sort_order) VALUES')
const pcValues = POSTCARD_QUESTIONS.map((p, i) =>
  `(${esc(p.id)}, ${esc(p.category === 'visual' ? 'visual' : p.category === 'cultural' ? 'cultural' : 'hyperlocal')}, ${esc(p.image || null)}, ${esc(p.question)}, ${jsonb(p.options)}, ${p.correct}, ${esc(p.category)}, ${esc(p.fact || null)}, ${i})`
)
lines.push(pcValues.join(',\n') + ';')
lines.push('')

// Write the file
const outPath = resolve(__dirname, 'p3_seed.sql')
writeFileSync(outPath, lines.join('\n'), 'utf-8')
console.log(`✅ Generated ${outPath}`)
console.log(`   Trivia: ${triviaRaw.length} rows`)
console.log(`   Crosswords: ${crosswords.length} rows`)
console.log(`   Puzzles: ${puzzles.length} rows`)
console.log(`   Campaign: ${campaign.length} rows`)
console.log(`   Coloring: ${coloring.length} rows`)
console.log(`   Adventures: ${advEntries.length} rows`)
console.log(`   Word Game: ${WORD_DATA.length} rows`)
console.log(`   Postcards: ${POSTCARD_QUESTIONS.length} rows`)
